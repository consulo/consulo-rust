...
public abstract class CargoDependencyCrateVisitor extends TomlVisitor {
    public abstract void visitDependency(@Nonnull DependencyCrate dependency);
    ...
    public void visitKeyValue(@Nonnull TomlKeyValue element) {
        ...
        if (value instanceof TomlLiteral && TomlLiteralExt.getKind((TomlLiteral) value) instanceof TomlLiteralKind.StringKind) {
    ...
    @Override
    public void visitTable(@Nonnull TomlTable element) {
    ...
    @Nonnull
    private static Map<String, TomlValue> collectProperties(@Nonnull TomlKeyValueOwner owner) {
        Map<String, TomlValue> result = new HashMap<>();
        for (TomlKeyValue entry : owner.getEntries()) {
            String keyName = TomlKeyExt.getName(entry.getKey());
    ...
    @Nullable
    private static TomlKeyValue getPackageKeyValue(@Nonnull TomlKeyValueOwner owner) {
        for (TomlKeyValue entry : owner.getEntries()) {
    ...
    static abstract class DependencyTable {
        ...
        static class Specific extends DependencyTable {
            ...
            Specific(@Nonnull String crateName, @Nonnull TomlElement crateNameElement) {
        ...
        @Nullable
        static DependencyTable fromTomlTable(@Nonnull TomlTable table) {
