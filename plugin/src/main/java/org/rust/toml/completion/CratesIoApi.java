...
public final class CratesIoApi {
    ...
    @Nonnull
    public static Collection<CrateDescription> searchCrate(@Nonnull TomlKeySegment key) {
        if (OpenApiUtil.isUnitTestMode()) {
            if (MOCK != null) return MOCK;
            return Collections.emptyList();
        }

        PsiElement original = CompletionUtilCore.getOriginalElement(key);
    ...
    @Nullable
    public static String getCrateLastVersion(@Nonnull TomlKeySegment key) {
        if (OpenApiUtil.isUnitTestMode()) {
    ...
    @Nullable
    private static <T> T requestCratesIo(@Nonnull PsiElement context, @Nonnull String path, @Nonnull Class<T> cls) {
        try {
            ...
            if (e instanceof consulo.component.ProcessCanceledException) {
                throw (consulo.component.ProcessCanceledException) e;
            }
            NotificationUtils.showBalloon(context.getProject(), RsBundle.message("notification.content.could.not.reach.crates.io"), NotificationType.WARNING);
            return null;
        }
    ...
    public static void withMockedCrateSearch(@Nonnull List<CrateDescription> mock, @Nonnull Runnable action) {
        MOCK = mock;
        try {
            action.run();
        } finally {
