...
public class CargoTomlPsiPattern {
    ...
    @Nonnull
    private static <I extends PsiElement> PsiElementPattern.Capture<I> cargoTomlPsiElement(@Nonnull Class<I> clazz) {
    ...
    @Nonnull
    public PsiElementPattern.Capture<PsiElement> inValueWithKey(@Nonnull String key) {
    ...
    private final PsiElementPattern.Capture<TomlTableHeader> myOnDependencyTableHeader =
        cargoTomlPsiElement(TomlTableHeader.class)
            .with(new PatternCondition<TomlTableHeader>("dependenciesCondition") {
                @Override
                public boolean accepts(@Nonnull TomlTableHeader header, ProcessingContext context) {
    ...
    @Nonnull
    public PsiElementPattern.Capture<TomlKeyValue> dependencyProperty(@Nonnull String name) {
        return PlatformPatterns.psiElement(TomlKeyValue.class)
            .with(new PatternCondition<TomlKeyValue>("name") {
                @Override
                public boolean accepts(@Nonnull TomlKeyValue e, ProcessingContext context) {
                    String keyName = TomlKeyExt.getName(e.getKey());
    ...
    private static PsiElementPattern.Capture<TomlLiteral> cargoTomlStringLiteral() {
        return cargoTomlPsiElement(TomlLiteral.class)
            .with(new PatternCondition<TomlLiteral>("stringLiteral") {
                @Override
                public boolean accepts(@Nonnull TomlLiteral e, ProcessingContext context) {
                    return TomlLiteralExt.getKind(e) instanceof TomlLiteralKind.StringKind;
                }
            });
    }

    @Nonnull
    private static PsiElementPattern.Capture<TomlKeyValue> tomlKeyValue(@Nonnull String key) {
    ...
    @Nonnull
    private static PsiElementPattern.Capture<TomlTable> tomlTable(@Nonnull String key) {
        return PlatformPatterns.psiElement(TomlTable.class).with(new PatternCondition<TomlTable>("WithName (" + key + ")") {
            @Override
            public boolean accepts(@Nonnull TomlTable e, ProcessingContext context) {
