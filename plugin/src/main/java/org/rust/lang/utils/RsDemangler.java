...
public final class RsDemangler {
    ...
    public static class Demangle {
        ...
        public Demangle(@Nonnull String original, @Nonnull String inner, @Nonnull String suffix,
     ...
     */
    @Nonnull
    public Demangle demangle(@Nonnull String name) {
        String text = name;
     ...
     */
    @Nullable
    public Demangle tryDemangle(@Nonnull String name) {
        Demangle sym = demangle(name);
        return sym.isValid() ? sym : null;
    }

    private static boolean isRustHash(@Nonnull String text) {
        return RUST_HASH_RE.matcher(text).matches();
    }

    private static boolean isSymbolLike(@Nonnull String text) {
    ...
    private static int parseInt(@Nonnull PeekableIterator<Character> iter) {
        int number = 0;
    ...
    @Nonnull
    private static String take(@Nonnull Iterator<Character> iter, int count) {
        StringBuilder sb = new StringBuilder();
    ...
    private static int takeCount(@Nonnull Iterator<Character> iter, int count) {
        int taken = 0;
        for (int i = 0; i < count; i++) {
    ...
    @Nonnull
    private static String remaining(@Nonnull Iterator<Character> iter) {
        StringBuilder sb = new StringBuilder();
        while (iter.hasNext()) {
            sb.append(iter.next());
        }
    ...
    @Nonnull
    private static Iterator<Character> toCharIterator(@Nonnull String s) {
