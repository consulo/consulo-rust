...
public class PerNsHashMap<K> extends THashMapBase<K, PerNs> {
    ...
    private Object[] items = ArrayUtil.EMPTY_OBJECT_ARRAY;
    private byte[] masks = ArrayUtil.EMPTY_BYTE_ARRAY;

    public PerNsHashMap(@Nonnull ModData containingMod, @Nonnull ModData rootMod) {
    ...
    @Override
    protected void setValueAtIndex(int index, @Nonnull PerNs value) {
        consulo.util.lang.Pair<VisItem, Namespace> single = PerNsHashMapUtil.asSingleVisItem(value);
        if (single == null) {
            items[index] = value;
            return;
        }
    ...
    @Nullable
    private Byte encodeMask(@Nonnull VisItem visItem, @Nonnull Namespace namespace) {
        int namespaceMask;
