...
public abstract class RsImplItemImplMixin extends RsStubbedElementImpl<RsImplItemStub> implements RsImplItem {
    ...
    public RsImplItemImplMixin(@Nonnull ASTNode node) {
        super(node);
    }

    public RsImplItemImplMixin(@Nonnull RsImplItemStub stub, @Nonnull IStubElementType nodeType) {
        super(stub, nodeType);
    }
    ...
    public CachedValue<RsCachedImplItem> getCachedImplItem() {
        return consulo.application.util.CachedValuesManager.getManager(getProject()).createCachedValue(() -> {
