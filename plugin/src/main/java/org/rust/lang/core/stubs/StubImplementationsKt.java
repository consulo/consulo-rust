...
public final class StubImplementationsKt {
    ...
    @Nonnull
    public static RsStubElementType<?, ?> factory(@Nonnull String name) {
        switch (name) {
    ...
    @Nullable
    static String readNameAsString(@Nonnull StubInputStream dataStream) throws IOException {
        consulo.index.io.StringRef ref = dataStream.readName();
        return ref != null ? ref.getString() : null;
    }

    @Nullable
    static String readUTFFastAsNullable(@Nonnull StubInputStream dataStream) throws IOException {
        return DataInputOutputUtil.readNullable(dataStream, dataStream::readUTFFast);
    }

    static void writeUTFFastAsNullable(@Nonnull StubOutputStream dataStream, @Nullable String value) throws IOException {
        DataInputOutputUtil.writeNullable(dataStream, value, dataStream::writeUTFFast);
    }

    @Nullable
    static Long readLongAsNullable(@Nonnull StubInputStream dataStream) throws IOException {
        return DataInputOutputUtil.readNullable(dataStream, dataStream::readLong);
    }

    static void writeLongAsNullable(@Nonnull StubOutputStream dataStream, @Nullable Long value) throws IOException {
        DataInputOutputUtil.writeNullable(dataStream, value, dataStream::writeLong);
    }

    @Nullable
    static Double readDoubleAsNullable(@Nonnull StubInputStream dataStream) throws IOException {
        return DataInputOutputUtil.readNullable(dataStream, dataStream::readDouble);
    }

    static void writeDoubleAsNullable(@Nonnull StubOutputStream dataStream, @Nullable Double value) throws IOException {
    ...
    static boolean isFunctionBody(@Nonnull ASTNode node) {
        return node.getElementType() == BLOCK && node.getTreeParent() != null && node.getTreeParent().getElementType() == FUNCTION;
    }

    static boolean shouldCreateExprStub(@Nonnull ASTNode node) {
    ...
    static boolean shouldCreateStmtStub(@Nonnull ASTNode node) {
