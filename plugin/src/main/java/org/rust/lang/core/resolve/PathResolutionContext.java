...
public class PathResolutionContext {
    ...
    @Nullable
    public RsModInfo getContainingModInfo(@Nonnull RsMod knownContainingMod) {
        if (containingModInfoInitialized) {
            return containingModInfo;
        }
        RsMod original = CompletionUtilCore.getOriginalOrSelf(knownContainingMod);
        if (containingModInitialized) {
    ...
    @Nonnull
    public RsPathResolveKind classifyPath(@Nonnull RsPath path) {
    ...
    @Nonnull
    private RsPathResolveKind classifyUnqualifiedPath(@Nonnull RsPath path, @Nonnull Set<Namespace> ns) {
    ...
    private static boolean rootPathParentIsUseSpeck(@Nonnull RsPath path) {
        RsPath root = path;
    ...
    private static boolean rootPathParentIsVisRestriction(@Nonnull RsPath path) {
        RsPath root = path;
        while (root.getPath() != null) {
            root = root.getPath();
        }
