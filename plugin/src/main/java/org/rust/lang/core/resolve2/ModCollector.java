...
public final class ModCollector {
    ...
    public static Map<String, DeclMacroDefInfo> collectScope(
        ...
        @Nonnull ModCollectorContext context
    ) {
    ...
    public static Map<String, DeclMacroDefInfo> collectScope(
        ...
        @Nonnull ModData modData,
        @Nonnull ModCollectorContext context,
        @Nonnull MacroIndex modMacroIndex,
    ...
    public static void collectExpandedElements(
        @Nonnull StubElement<? extends RsItemsOwner> scope,
        @Nonnull MacroCallInfo call,
        @Nonnull ModCollectorContext context,
        @Nullable DollarCrateHelper dollarCrateHelper
    ) {
        ModCollectorImpl collector = new ModCollectorImpl(
    ...
    private static StubElement<? extends RsItemsOwner> getOrBuildStub(@Nonnull RsItemsOwner owner) {
        ...
        if (owner instanceof RsModItem) {
            // PsiAnchor.calcStubIndex is not available in Consulo API; skip fine-grained stub-index lookup.
            return null;
        }
        return null;
    }
     ...
     */
    @Nullable
    public static RsFileStub getOrBuildFileStub(@Nonnull RsFileBase file) {
        Object greenStubTree = file.getGreenStubTree();
        StubElement<?> stub;
        if (greenStubTree != null) {
            stub = ((consulo.language.psi.stub.StubTree) greenStubTree).getRoot();
     ...
     */
    public static void importExternCrateMacros(@Nonnull CrateDefMap defMap, @Nonnull String externCrateName) {
     ...
     */
    public static void recordChildFileInUnusualLocation(@Nonnull ModData parent, int childFileId) {
        ...
        for (ModData modData : parent.getParentsIterable()) {
            ...
            if (VirtualFileUtil.isAncestor(containedDirectory, childDirectory, false)) return;
            VirtualFile commonAncestor = VirtualFileUtil.getCommonAncestor(containedDirectory, childDirectory);
    ...
    private static class ModCollectorImpl implements ModVisitor {
        ...
        @Nonnull private final ModCollectorContext context;
        @Nonnull private final MacroIndex parentMacroIndex;
        ...
        @Nonnull final Map<String, DeclMacroDefInfo> legacyMacros = new HashMap<>();

        ModCollectorImpl(
            @Nonnull ModData modData,
            @Nonnull ModCollectorContext context,
            @Nonnull MacroIndex parentMacroIndex,
        ...
        void collectMod(@Nonnull StubElement<?> mod, boolean propagateLegacyMacros) {
            ModVisitor visitor;
        ...
        @Nonnull
        private VisItem convertToVisItem(@Nonnull ItemLight item, boolean isModOrEnum, boolean forceCfgDisabledVisibility) {
        ...
        @Nullable
        private ModData tryCollectChildModule(@Nonnull ModOrEnumItemLight item, @Nonnull RsNamedStub stub, int index) {
        ...
        private ModData collectChildModInline(
            @Nonnull RsModItemStub modStub,
            @Nonnull ModOrEnumItemLight item,
            int index,
        ...
        private ModData collectChildModFile(
            @Nonnull ModOrEnumItemLight item,
            @Nonnull RsModDeclItemStub stub,
            int index,
            @Nullable VirtualFile parentOwnedDirectory
        ) {
        ...
        @Nonnull
        private ModData collectEnumAsModData(@Nonnull ModOrEnumItemLight enumItem, @Nonnull RsEnumItemStub enumStub) {
        ...
        private void propagateLegacyMacros(@Nonnull ModData modData) {
        ...
        @Nonnull
        private Visibility convertVisibility(@Nonnull VisibilityLight visibility, boolean isDeeplyEnabledByCfg) {
    ...
    @Nullable
    private static Visibility.Restricted resolveRestrictedVisibility(@Nonnull String[] path, @Nonnull ModData containingMod) {
    ...
    @Nullable
    private static VirtualFile getOwnedDirectory(@Nonnull ModData modData) {
        Integer ownedDirectoryId = modData.getOwnedDirectoryId();
        if (ownedDirectoryId == null) return null;
        return ManagingFS.getInstance().findFileById(ownedDirectoryId);
    }

    @Nullable
    private static VirtualFile asVirtualFile(@Nonnull ModData modData) {
        if (!modData.isRsFile()) throw new IllegalStateException("Not an RsFile");
        Integer fileId = modData.getFileId();
        if (fileId == null) return null;
        VirtualFile f = ManagingFS.getInstance().findFileById(fileId);
        if (f == null) {
    ...
    private static class DelegatingModVisitor implements ModVisitor {
        @Nonnull private final ModVisitor first;
        @Nonnull private final ModVisitor second;

        DelegatingModVisitor(@Nonnull ModVisitor first, @Nonnull ModVisitor second) {
            this.first = first;
            this.second = second;
        }
