...
final class BlockMayHaveStubsHeuristic {
    ...
    public static boolean computeAndCache(@Nonnull ASTNode node) {
        assertIsBlock(node);
    ...
    public static boolean getAndClearCached(@Nonnull ASTNode node) {
        assertIsBlock(node);
    ...
    private static void assertIsBlock(@Nonnull ASTNode node) {
        if (node.getElementType() != BLOCK) {
            throw new IllegalStateException("Expected block, found: " + node.getElementType() + ", text: `" + node.getText() + "`");
        }
    }

    private static boolean computeBlockMayHaveStubs(@Nonnull ASTNode node) {
        consulo.language.parser.PsiBuilder b = PsiBuilderFactory.getInstance()
