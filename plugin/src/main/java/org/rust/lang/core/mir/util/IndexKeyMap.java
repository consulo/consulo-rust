...
public class IndexKeyMap<K extends WithIndex, V> implements Map<K, V> {
    ...
    private IndexKeyMap(@Nonnull List<V> inner) {
        this.inner = inner;
    }
    ...
    @Nonnull
    public V getOrPut(@Nonnull K key, @Nonnull Supplier<V> defaultValue) {
        V value = get(key);
        if (value == null) {
    ...
    @SuppressWarnings("unchecked")
    @Nonnull
    public static <K extends WithIndex, V> Map<K, V> fromListUnchecked(@Nonnull List<V> list) {
