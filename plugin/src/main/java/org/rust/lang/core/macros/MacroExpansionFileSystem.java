...
public class MacroExpansionFileSystem extends NewVirtualFileSystem {
    ...
    public boolean isValidName(@Nonnull String name) {
        return !name.contains("/") && !name.contains("\\") && !name.isEmpty();
    }
    ...
    @Nonnull
    @Override
    public VirtualFile copyFile(@Nullable Object requestor, @Nonnull VirtualFile file, @Nonnull VirtualFile newParent, @Nonnull String copyName) throws IOException {
        throw new UnsupportedOperationException();
    }

    @Override
    public void deleteFile(@Nullable Object requestor, @Nonnull VirtualFile file) throws IOException {
        if (requestor != TrustedRequestor.INSTANCE) {
            throw new UnsupportedOperationException();
        }
    ...
    @Override
    public void moveFile(@Nullable Object requestor, @Nonnull VirtualFile file, @Nonnull VirtualFile newParent) throws IOException {
        if (requestor != TrustedRequestor.INSTANCE) {
            throw new UnsupportedOperationException();
        }
    ...
    @Override
    public void renameFile(@Nullable Object requestor, @Nonnull VirtualFile file, @Nonnull String newName) throws IOException {
        if (requestor != TrustedRequestor.INSTANCE) {
            throw new UnsupportedOperationException();
        }
        FSItem fsItem = convert(file);
    ...
    @Nullable
    private FSItem convert(@Nonnull VirtualFile file) {
    ...
    @Nullable
    private FSItem convert(@Nonnull String path, boolean mkdirs) {
        FSItem file = root;
    ...
    @Nullable
    private FSItem convert(@Nonnull String path) {
    ...
    @Override
    public void setWritable(@Nonnull VirtualFile file, boolean writableFlag) {
    }
    ...
    public byte[] contentsToByteArray(@Nonnull VirtualFile file) throws IOException {
        ...
        if (mixHash != null && storedVersion == ExpandedMacroStorage.MACRO_STORAGE_VERSION) {
            ...
            if (expansion != null) {
                var ok = expansion.ok();
                if (ok != null) {
                    return String.valueOf(ok.getText()).getBytes();
                }
            }
        }
    ...
    @Nonnull
    @Override
    public OutputStream getOutputStream(@Nonnull VirtualFile file, @Nullable Object requestor, long modStamp, long timeStamp) throws IOException {
    ...
    public FileAttributes getAttributes(@Nonnull VirtualFile file) {
        ...
        return new FileAttributes(isDir, false, false, false, length, item.getTimestamp(), true);
    }
    ...
    public static abstract class FSItem {
        ...
        protected FSItem(@Nullable FSDir parent, @Nonnull String name, long timestamp) {
            myParent = parent;
            myName = name;
        ...
        void buildPath(@Nonnull StringBuilder sb) {
            FSDir parent = getParent();
            if (parent != null) {
                parent.buildPath(sb);
            }
    ...
    public static class FSDir extends FSItem {
        ...
        public FSDir(@Nullable FSDir parent, @Nonnull String name) {
            this(parent, name, currentTimestamp());
        }

        public FSDir(@Nullable FSDir parent, @Nonnull String name, long timestamp) {
         ...
         */
        protected FSDir(@Nullable FSDir parent, @Nonnull String name, long timestamp, boolean customChildren) {
            super(parent, name, timestamp);
        ...
        @Nullable
        public synchronized FSItem findChild(@Nonnull String name) {
        ...
        public synchronized void addChild(@Nonnull FSItem item, boolean bump, boolean override) {
        ...
        public void addChild(@Nonnull FSItem item, boolean bump) {
            addChild(item, bump, false);
        }

        public void addChild(@Nonnull FSItem item) {
            addChild(item, false, false);
        }

        @Nonnull
        public FSFile addChildFile(@Nonnull String name, boolean bump) {
        ...
        @Nonnull
        public FSFile addChildFile(@Nonnull String name) {
            return addChildFile(name, false);
        }

        @Nonnull
        public FSDir addChildDir(@Nonnull String name, boolean bump) {
        ...
        @Nonnull
        public FSDir addChildDir(@Nonnull String name) {
            return addChildDir(name, false);
        }

        public synchronized void removeChild(@Nonnull String name, boolean bump) {
    ...
    public static class DummyDir extends FSDir {
        public DummyDir(@Nullable FSDir parent, @Nonnull String name, long timestamp) {
            super(parent, name, timestamp, true);
        }

        public DummyDir(@Nullable FSDir parent, @Nonnull String name) {
            this(parent, name, currentTimestamp());
        }
    ...
    public static class FSFile extends FSItem {
        ...
        public FSFile(@Nonnull FSDir parent, @Nonnull String name) {
            this(parent, name, currentTimestamp(), 0, null);
        }

        public FSFile(@Nonnull FSDir parent, @Nonnull String name, long timestamp, int length, @Nullable byte[] tempContent) {
            super(parent, name, timestamp);
            myLength = length;
        ...
        public synchronized void setContent(@Nonnull byte[] content) {
            myTempContent = content;
    ...
    @Nonnull
    private String[] splitFilenameAndParent(@Nonnull String path) {
    ...
    public void createFileWithExplicitContent(@Nonnull String path, @Nonnull byte[] content, boolean mkdirs) {
        createFileWithoutContent(path, mkdirs).setContent(content);
    }

    public void createFileWithExplicitContent(@Nonnull String path, @Nonnull byte[] content) {
        createFileWithExplicitContent(path, content, false);
    }

    public void createFileWithImplicitContent(@Nonnull String path, int fileSize, boolean mkdirs) {
        createFileWithoutContent(path, mkdirs).setImplicitContent(fileSize);
    }

    public void createFileWithImplicitContent(@Nonnull String path, int fileSize) {
        createFileWithImplicitContent(path, fileSize, false);
    }

    @Nonnull
    public FSFile createFileWithoutContent(@Nonnull String path, boolean mkdirs) {
    ...
    @Nonnull
    public FSFile createFileWithoutContent(@Nonnull String path) {
        return createFileWithoutContent(path, false);
    }

    public void setFileContent(@Nonnull String path, @Nonnull byte[] content) {
    ...
    public void createDirectoryIfNotExistsOrDummy(@Nonnull String path) {
        String[] parts = splitFilenameAndParent(path);
        String parentName = parts[0];
        String name = parts[1];
    ...
    public void setDirectory(@Nonnull String path, @Nonnull FSDir dir, boolean override) {
        String[] parts = splitFilenameAndParent(path);
        String parentName = parts[0];
        String name = parts[1];
        FSItem parent = convert(parentName);
    ...
    public void setDirectory(@Nonnull String path, @Nonnull FSDir dir) {
        setDirectory(path, dir, true);
    }

    public void makeDummy(@Nonnull String path) {
    ...
    @Nullable
    public FSDir getDirectory(@Nonnull String path) {
        FSItem item = convert(path);
        return item instanceof FSDir ? (FSDir) item : null;
    }

    public void deleteFilePath(@Nonnull String path) {
        String[] parts = splitFilenameAndParent(path);
        String parentName = parts[0];
        String name = parts[1];
        FSItem parent = convert(parentName);
    ...
    public void cleanDirectoryIfExists(@Nonnull String path, boolean bump) {
    ...
    public void cleanDirectoryIfExists(@Nonnull String path) {
    ...
    public static void writeFSItem(@Nonnull DataOutput data, @Nonnull FSItem item) throws IOException {
    ...
    @Nonnull
    public static FSItem readFSItem(@Nonnull DataInput data, @Nullable FSDir parent) throws IOException {
    ...
    public static <T> T withAllowedWriting(@Nonnull VirtualFile file, @Nonnull java.util.function.Supplier<T> f) {
    ...
    public static boolean isWritingAllowed(@Nonnull VirtualFile file) {
        return Boolean.TRUE.equals(file.getUserData(RUST_MACROS_ALLOW_WRITING));
    }

    private static void setAllowWriting(@Nonnull VirtualFile file, boolean allow) {
