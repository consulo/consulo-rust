...
public class MirBorrowCheckVisitor implements ResultsVisitor<BorrowCheckResults.State> {
    ...
    public void visitStatementBeforePrimaryEffect(
        @Nonnull BorrowCheckResults.State state,
        @Nonnull MirStatement statement,
        @Nonnull MirLocation location
    ) {
    ...
    public void visitTerminatorBeforePrimaryEffect(
        @Nonnull BorrowCheckResults.State state,
        @Nonnull MirTerminator<MirBasicBlock> terminator,
        @Nonnull MirLocation location
    ) {
        checkActivations(location, state);
    }
    ...
    private void mutatePlace(@Nonnull MirLocation location, @Nonnull MirPlace place, @Nonnull BorrowCheckResults.State state) {
        // TODO: checkIfAssignedPathIsMoved
        accessPlace(location, place, new MirReadOrWrite.Write(MirWriteKind.Mutate.INSTANCE), state);
    }

    private void consumeRvalue(@Nonnull MirLocation location, @Nonnull MirRvalue rvalue, @Nonnull BorrowCheckResults.State state) {
    ...
    private void consumeOperand(@Nonnull MirLocation location, @Nonnull MirOperand operand, @Nonnull BorrowCheckResults.State state) {
    ...
    private void accessPlace(
        ...
        @Nonnull MirReadOrWrite readOrWrite,
        @Nonnull BorrowCheckResults.State state
    ) {
    ...
    private void reportMoveOutWhileBorrowed(@Nonnull MirLocation location) {
    ...
    private void checkActivations(@Nonnull MirLocation location, @Nonnull BorrowCheckResults.State state) {
    ...
    private void checkIfFullPathIsMoved(@Nonnull MirLocation location, @Nonnull MirPlace place, @Nonnull BorrowCheckResults.State state) {
    ...
    private void checkIfPathOrSubpathIsMoved(@Nonnull MirLocation location, @Nonnull MirPlace place, @Nonnull BorrowCheckResults.State state) {
    ...
    @Nonnull
    private MovePath movePathClosestTo(@Nonnull MirPlace place) {
     ...
     */
    @Nullable
    private MovePath movePathForPlace(@Nonnull MirPlace place) {
        LookupResult result = moveData.getRevLookup().find(place);
        if (result instanceof LookupResult.Exact) {
            return ((LookupResult.Exact) result).getMovePath();
        } else if (result instanceof LookupResult.Parent) {
    ...
    private void reportUseOfMovedOrUninitialized(
        @Nonnull MirLocation location,
        @Nonnull MirPlace movedPlace,
        @Nonnull MirPlace usedPlace,
    ...
    @Nonnull
    private List<MoveOut> getMovedIndexes(@Nonnull MirLocation location, @Nonnull MovePath movePath) {
    ...
    private boolean dfsIter(
        @Nonnull MirLocation location,
        @Nonnull List<MovePath> movePaths,
        @Nonnull MovePath movePath,
        @Nonnull Set<MirLocation> visited,
        @Nonnull List<MoveOut> result
    ) {
    ...
    @Nonnull
    private static List<MirLocation> predecessorLocations(@Nonnull MirBody body, @Nonnull MirLocation location) {
