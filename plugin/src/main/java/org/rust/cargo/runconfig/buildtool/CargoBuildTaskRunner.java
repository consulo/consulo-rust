...
public class CargoBuildTaskRunner extends ProjectTaskRunner {
    ...
    public Promise<Result> run(@Nonnull Project project, @Nonnull ProjectTaskContext context, @Nonnull ProjectTask... tasks) {
        ...
        if (configuration instanceof CargoCommandConfiguration cargoConfig) {
            if (org.rust.cargo.runconfig.RunConfigUtil.getHasRemoteTarget(cargoConfig) ||
                ...
                consulo.application.ApplicationManager.getApplication().invokeLater(() ->
        ...
        CargoBuildSessionsQueueManager.getInstance(project)
            ...
            .run(queuedTask, ModalityState.nonModal(), new EmptyProgressIndicator());

        return resultPromise;
    }

    @Override
    public boolean canRun(@Nonnull ProjectTask projectTask) {
        ...
        if (projectTask instanceof ModuleBuildTask moduleBuildTask) {
            ...
            consulo.execution.RunnerAndConfigurationSettings selected = runManager.getSelectedConfiguration();
            if (selected == null) return false;
            consulo.execution.configuration.RunConfiguration buildableElement = selected.getConfiguration();
    ...
    public List<ProjectTask> expandTask(ProjectTask task) {
        ...
        for (CargoProject cargoProject : allProjects) {
            ...
            consulo.execution.RunnerAndConfigurationSettings runSettings = RunConfigUtil.createCargoCommandRunConfiguration(runManager, cmdLine, null);
            ExecutionEnvironment env = new ExecutionEnvironment(executor, runner, runSettings, project);
            consulo.execution.configuration.RunConfiguration config = runSettings.getConfiguration();
