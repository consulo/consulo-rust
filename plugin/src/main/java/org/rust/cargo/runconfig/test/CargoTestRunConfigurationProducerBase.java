...
public abstract class CargoTestRunConfigurationProducerBase extends CargoRunConfigurationProducer {
    ...
    protected void registerConfigProvider(@Nonnull BiFunction<List<PsiElement>, Boolean, TestConfig> provider) {
        testConfigProviders.add(provider);
    }

    protected void registerDirectoryConfigProvider(@Nonnull Function<PsiDirectory, TestConfig> provider) {
    ...
    @Nullable
    public TestConfig findTestConfig(@Nonnull List<PsiElement> elements, boolean climbUp) {
    ...
    @Nullable
    public TestConfig findTestConfig(@Nonnull List<PsiElement> elements) {
        return findTestConfig(elements, true);
    }

    @Nullable
    private TestConfig findTestConfig(@Nonnull ConfigurationContext context) {
        PsiElement[] array = context.getDataContext().getData(LangDataKeys.PSI_ELEMENT_ARRAY);
    ...
    @Nullable
    protected TestConfig createConfigForDirectory(@Nonnull PsiDirectory dir) {
    ...
    @Nullable
    protected TestConfig createConfigForMultipleFiles(@Nonnull List<PsiElement> elements, boolean climbUp) {
    ...
    protected TestConfig createConfigForQualifiedElement(
        ...
        boolean climbUp,
        @Nonnull Class<? extends RsQualifiedNamedElement> targetClass
    ) {
    ...
    @Nullable
    protected TestConfig createConfigForMod(@Nonnull List<PsiElement> elements, boolean climbUp) {
        return createConfigForQualifiedElement(elements, climbUp, RsMod.class);
    }

    @SuppressWarnings("unchecked")
    @Nullable
    protected <T extends PsiElement> T findElement(@Nonnull PsiElement base, boolean climbUp, @Nonnull Class<T> targetClass) {
        if (targetClass.isInstance(base)) return (T) base;
        if (!climbUp) return null;
        return consulo.language.psi.util.PsiTreeUtil.getParentOfType(base, targetClass, false);
    }
    ...
    protected boolean isIgnoredTest(@Nonnull PsiElement element) {
    ...
    private static String escapePathFragment(@Nonnull String fragment) {
    ...
    @Nullable
    public static RsQualifiedNamedElement getSourceElement(@Nonnull RsQualifiedNamedElement element) {
    ...
    @Nonnull
    private static List<RsFile> getDirectoryTargets(@Nonnull PsiDirectory dir) {
    ...
    @Nullable
    private static PsiDirectory getSourceRoot(@Nonnull PsiDirectory dir) {
    ...
    private static boolean isSourceRoot(@Nonnull PsiDirectory dir) {
    ...
    private static class DirectoryTestConfig implements TestConfig {
        ...
        @Nonnull private final List<CargoWorkspace.Target> targets;
        @Nonnull private final PsiDirectory sourceElement;

        DirectoryTestConfig(
            @Nonnull String commandName,
            @Nonnull List<CargoWorkspace.Target> targets,
            @Nonnull PsiDirectory sourceElement
        ) {
        ...
        @Nonnull @Override public List<CargoWorkspace.Target> getTargets() { return targets; }
    ...
    private static class MultipleFileTestConfig implements TestConfig {
        @Nonnull private final String commandName;
        @Nonnull private final List<CargoWorkspace.Target> targets;
        @Nonnull private final PsiElement sourceElement;
