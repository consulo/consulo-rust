 ...
 * <p>
 * Can't use {@link consulo.execution.ui.console.RegexpFilter} directly because it doesn't handle
...
public class RegexpFileLinkFilter implements Filter, DumbAware {
    ...
    public RegexpFileLinkFilter(@Nonnull Project project,
                                @Nonnull VirtualFile cargoProjectDirectory,
                                @Nonnull String lineRegExp) {
    ...
    @Nullable
    public Matcher matchLine(@Nonnull String line) {
        Matcher matcher = myLinePattern.matcher(line);
        return matcher.matches() ? matcher : null;
    }

    private int zeroBasedNumber(@Nonnull String number) {
        try {
    ...
    @Nullable
    private ResolvedPath resolveFilePath(@Nonnull String fileName) {
    ...
    @Nullable
    private ResolvedPath resolveCargoPath(@Nonnull String path) {
    ...
    @Nullable
    private ResolvedPath resolveStdlibPath(@Nonnull String path) {
    ...
    @Nonnull
    private String normalizeStdLibPath(@Nonnull String path) {
    ...
    public static abstract class ResolvedPath {
        ...
        protected ResolvedPath(@Nonnull VirtualFile file) {
            myFile = file;
        }
        ...
        public static class Workspace extends ResolvedPath {
            public Workspace(@Nonnull VirtualFile file) {
                super(file);
            }
        }

        public static class Stdlib extends ResolvedPath {
            public Stdlib(@Nonnull VirtualFile file) {
                super(file);
            }
        }

        public static class CargoDependency extends ResolvedPath {
            public CargoDependency(@Nonnull VirtualFile file) {
                super(file);
            }
        }

        public static class Unknown extends ResolvedPath {
            public Unknown(@Nonnull VirtualFile file) {
                super(file);
            }
        }
    }
