...
public class CargoTestRunLineMarkerContributor extends RunLineMarkerContributor {
    ...
    public Info getInfo(@Nonnull PsiElement element) {
        ...
        consulo.ui.image.Image icon = getTestStateIcon(target);
        if (icon == null) {
            icon = AllIcons.RunConfigurations.Application;
        }

        consulo.ui.image.Image finalIcon = icon;
        return new Info(
            finalIcon,
            psiElement -> "",
    ...
    @Nullable
    public static consulo.ui.image.Image getTestStateIcon(@Nonnull PsiElement sourceElement) {
        String url;
