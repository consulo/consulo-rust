...
public class RsUnusedImportInspection extends RsLintInspection {
    ...
    public RsVisitor buildVisitor(@Nonnull RsProblemsHolder holder, boolean isOnTheFly) {
        return new RsWithMacrosInspectionVisitor() {
            ...
            @Override
            public void visitUseItem2(@Nonnull RsUseItem item) {
    ...
    private void storeUseSpeckUsage(
        ...
        @Nonnull PathUsageMap usage,
        @Nonnull Set<RsUseSpeck> unusedSpeckSet
    ) {
    ...
    private void markAsUnused(@Nonnull RsUseSpeck useSpeck, @Nonnull RsProblemsHolder holder) {
    ...
    @Nonnull
    private static PsiElement getHighlightElement(@Nonnull RsUseSpeck useSpeck) {
    ...
    private static boolean isApplicableForUseItem(@Nonnull RsUseItem item) {
     ...
     */
    public static boolean isUseSpeckUsed(@Nonnull RsUseSpeck useSpeck, @Nonnull PathUsageMap pathUsage) {
    ...
    private static boolean isUseSpeckUsedInternal(@Nonnull RsUseSpeck useSpeck, @Nonnull PathUsageMap usage) {
    ...
    private static boolean isItemUsedInSameMod(@Nonnull RsNamedElement item, @Nonnull String importName, @Nonnull PathUsageMap usage) {
    ...
    private static boolean isItemUsedInOtherMods(@Nonnull RsNamedElement item, @Nonnull String importName, @Nonnull RsUseSpeck useSpeck) {
        RsUseItem useItem = RsElementUtil.ancestorStrict(useSpeck, RsUseItem.class);
        if (useItem == null) return true;
    ...
    private static boolean isImportNeededForReference(
        @Nonnull RsReferenceElement reference,
        @Nonnull RsVisibility.Restricted importVisibility,
        @Nonnull RsMod importMod
    ) {
    ...
    private static boolean hasTransitiveGlobImportTo(@Nonnull RsMod self, @Nonnull RsMod target) {
    ...
    public static boolean processReferencesWithAliases(
        ...
        @Nonnull ReferenceProcessor originalProcessor
    ) {
    ...
    private static boolean checkProcMacrosMatch(@Nonnull InspectionProfile profile, @Nonnull Project project) {
        if (OpenApiUtil.isUnitTestMode()) return true;
        consulo.language.editor.inspection.scheme.InspectionToolWrapper<?> toolWrapper = profile.getInspectionTool(SHORT_NAME, project);
        if (toolWrapper == null) return true;
        consulo.language.editor.inspection.InspectionTool tool = toolWrapper.getTool();
    ...
    public interface ReferenceProcessor {
        boolean process(@Nonnull RsReferenceElement element);
    }
    ...
    private static class NamedItem {
        ...
        NamedItem(@Nonnull String importName, @Nonnull RsNamedElement item) {
    ...
    public consulo.localize.LocalizeValue getDisplayName() {
        return consulo.localize.LocalizeValue.of(org.rust.RsBundle.message("inspection.rs.unused.import.display.name"));
    }
