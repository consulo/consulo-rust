...
public class RsProblemsHolder {
    ...
    public RsProblemsHolder(@Nonnull ProblemsHolder holder) {
            ...
            if (element.getContainingFile() == getFile()) {
                ...
                if (result == null) {
                    if (alwaysShowInMacros) {
                        consulo.language.psi.PsiElement macroCall = RsExpandedElementUtil.findMacroCallExpandedFromNonRecursive(element);
    ...
    private boolean isProblemWithTypeAllowed(@Nonnull ProblemHighlightType highlightType) {
        return highlightType != ProblemHighlightType.INFORMATION || myHolder.isOnTheFly();
    }

    @Nullable
    private PsiElement findCorrespondingElementExpandedFrom(@Nonnull PsiElement element) {
    ...
    @Nonnull
    private LocalQuickFix[] filterSupportingMacros(@Nonnull LocalQuickFix[] fixes) {
        if (!isOnTheFly()) {
