...
import org.rust.lang.utils.PreparedAnnotation;
...
public abstract class RsDiagnostic {
    ...
    /**
     * Returns the presentable diagnostic for a problem reported by the language layer, or
     * {@code null} when no diagnostic is defined for it.
     */
    @Nullable
    public static RsDiagnostic of(@Nonnull RsInferenceDiagnostic diagnostic) {
        if (diagnostic instanceof RsTypeMismatch) {
            RsTypeMismatch mismatch = (RsTypeMismatch) diagnostic;
            return new TypeError(mismatch.getElement(), mismatch.getExpectedTy(), mismatch.getActualTy());
        }
        if (diagnostic instanceof RsFeatureDiagnostic) {
            return of((RsFeatureDiagnostic) diagnostic, Collections.emptyList());
        }
        return null;
    }

    /**
     * Returns the presentable diagnostic for a compiler feature that cannot be used, offering
     * {@code extraFixes} in addition to the fix that declares the feature in the crate root.
     */
    @Nonnull
    public static RsDiagnostic of(@Nonnull RsFeatureDiagnostic diagnostic, @Nonnull List<LocalQuickFix> extraFixes) {
        List<LocalQuickFix> fixes = new ArrayList<>(extraFixes);
        if (diagnostic.canAddFeatureAttribute()) {
            fixes.add(new AddFeatureAttributeFix(diagnostic.getFeatureName(), diagnostic.getElement()));
        }
        if (diagnostic.getKind() == RsFeatureDiagnostic.Kind.REMOVED) {
            return new RemovedFeature(diagnostic.getElement(), diagnostic.getEndElement(), diagnostic.getMessage(), fixes);
        }
        return new ExperimentalFeature(diagnostic.getElement(), diagnostic.getEndElement(), diagnostic.getMessage(), fixes);
    }
    ...
    public static void addToHolder(@Nonnull RsDiagnostic diagnostic, @Nonnull RsAnnotationHolder holder,
        ...
        if (!checkExistsAfterExpansion || org.rust.lang.core.psi.ext.impl.RsElementUtil.existsAfterExpansion(diagnostic.myElement)) {
