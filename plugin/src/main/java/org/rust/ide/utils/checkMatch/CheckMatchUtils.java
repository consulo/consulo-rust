...
public final class CheckMatchUtils {
    ...
    @Nullable
    public static List<Pattern> checkExhaustive(@Nonnull RsMatchExpr matchExpr) {
    ...
    @Nullable
    private static List<Pattern> doCheckExhaustive(@Nonnull RsMatchExpr match) throws CheckMatchException {
     ...
     */
    public static boolean isWellTyped(@Nonnull List<List<Pattern>> matrix) {
    ...
    @Nullable
    public static Ty getFirstColumnType(@Nonnull List<List<Pattern>> matrix) throws CheckMatchException {
    ...
    @Nonnull
    public static List<Pattern> getFirstColumn(@Nonnull List<List<Pattern>> matrix) {
        List<Pattern> result = new ArrayList<>();
     ...
     */
    @Nullable
    public static List<List<Pattern>> calculateMatrix(@Nonnull List<? extends RsMatchArm> arms) {
    ...
    @Nonnull
    static Pattern lowerPat(@Nonnull RsPat pat) throws CheckMatchException {
        return new Pattern(RsTypesUtil.getType(pat), getPatKind(pat));
    }

    @Nonnull
    private static PatternKind getPatKind(@Nonnull RsPat pat) throws CheckMatchException {
        if (pat instanceof RsPatIdent) {
            ...
            consulo.language.psi.PsiElement resolved = binding.getReference().resolve();
        ...
        if (pat instanceof RsPatStruct) {
            ...
            consulo.language.psi.PsiElement resolved = patStruct.getPath().getReference().resolve();
        ...
        if (pat instanceof RsPatTupleStruct) {
            ...
            consulo.language.psi.PsiElement resolved = patTupleStruct.getPath().getReference().resolve();
            if (resolved == null) {
        ...
        if (pat instanceof RsPatConst) {
            ...
            if (ty instanceof TyAdt) {
                ...
                if (adt.getItem() instanceof RsEnumItem) {
                    ...
                    consulo.language.psi.PsiElement resolved = path.getReference().resolve();
                    if (!(resolved instanceof RsEnumVariant)) {
    ...
    @Nonnull
    private static Pattern createPatternForField(@Nullable RsPatField patField, @Nonnull RsFieldDecl field) throws CheckMatchException {
        if (patField != null) {
    ...
    @Nonnull
    private static PatternKind getLeafOrVariant(@Nonnull RsElement item, @Nonnull List<Pattern> subPatterns) throws CheckMatchException {
    ...
    @Nullable
    private static ConstExpr.Value<?> getExprValue(@Nonnull RsExpr expr) {
    ...
    public static UsefulnessResult isUseful(
        @Nonnull List<List<Pattern>> matrix,
    ...
    private static void fillWithSubPatterns(@Nonnull List<Pattern> target, @Nonnull List<Pattern> subPatterns) {
    ...
    private static boolean isIntegral(@Nonnull Ty type) {
