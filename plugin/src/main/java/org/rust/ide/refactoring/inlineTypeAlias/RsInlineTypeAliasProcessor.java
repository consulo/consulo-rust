...
public class RsInlineTypeAliasProcessor extends BaseRefactoringProcessor {
    ...
    protected UsageInfo[] findUsages() {
        List<? extends consulo.language.psi.PsiReference> usages;
    ...
    private void addNeededImports(@Nonnull List<RsElement> inlined) {
        RsTypeReference typeReference = myTypeAlias.getTypeReference();
        if (typeReference == null) return;
    ...
    protected consulo.localize.LocalizeValue getCommandName() {
        ...
        return consulo.localize.LocalizeValue.of(RsBundle.message("command.name.inline.type.alias", name));
    }
    ...
    @Nullable
    public static Substitution tryGetTypeAliasSubstitutionUsingParent(@Nonnull RsPath path, @Nonnull RsTypeAlias typeAlias) {
    ...
    public static RsElement fillPathWithActualType(
        @Nonnull RsPath path,
        @Nonnull RsTypeReference typeReference,
    ...
    @Nullable
    private static RsElement fillPathWithActualTypeText(@Nonnull RsPath path, @Nonnull String typeText) {
    ...
    private static class PathUsage {
        ...
        PathUsage(@Nonnull RsPath path, @Nonnull Substitution substitution) {
            myPath = path;
