...
public class RsMoveCommonProcessor {
    ...
    @Nullable
    private RsMoveUsageInfo createMoveUsageInfo(@Nonnull PsiReference reference) {
    ...
    private boolean checkMacroCallPath(@Nonnull RsPath path) {
    ...
    @Nullable
    private RsMoveReferenceInfo createOutsideReferenceInfo(@Nonnull RsElement pathOriginal, @Nonnull RsQualifiedNamedElement target) {
    ...
    @Nonnull
    private List<RsMoveReferenceInfo> preprocessInsideReferences(@Nonnull UsageInfo[] usages) {
    ...
    @Nonnull
    private RsMoveReferenceInfo createInsideReferenceInfo(@Nonnull RsPath pathOriginal, @Nonnull RsQualifiedNamedElement target) {
        RsPath path = RsMoveUtil.convertFromPathOriginal(pathOriginal, codeFragmentFactory);
    ...
    @Nullable
    private RsMoveReferenceInfo convertToFullReference(@Nonnull RsMoveReferenceInfo reference) {
    ...
    @Nullable
    private RsPath convertPathToFull(@Nonnull RsPath pathOld, @Nonnull RsMoveReferenceInfo reference, @Nonnull RsPath path) {
    ...
    public void performRefactoring(@Nonnull UsageInfo[] usages, @Nonnull Supplier<List<ElementToMove>> moveElements) {
    ...
    private void restoreOutsideReferenceInfosAfterMove(@Nonnull Map<RsElement, RsElement> pathMapping) {
    ...
    private void updateInsideReferenceInfosIfNeeded(
        @Nonnull List<RsMoveReferenceInfo> references,
        @Nonnull Map<RsElement, RsElement> pathMapping
    ) {
    ...
    @Nonnull
    private <T extends RsElement> Map<T, T> createMapping(@Nonnull Key<T> key, @Nonnull Class<T> aClass) {
    ...
    private void restorePathOldAfterMove(@Nonnull RsMoveReferenceInfo reference, @Nonnull Map<RsElement, RsElement> pathMapping) {
    ...
    public void updateMovedItemVisibility(@Nonnull RsItemElement item) {
        ...
        if (visibility instanceof RsVisibility.Private) {
            if (conflictsDetector != null && conflictsDetector.getItemsToMakePublic().contains(item)) {
                ...
                consulo.language.psi.PsiFile containingFile = item.getContainingFile();
