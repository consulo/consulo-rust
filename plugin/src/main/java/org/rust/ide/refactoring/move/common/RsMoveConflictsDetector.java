...
public class RsMoveConflictsDetector {
    ...
    @Nonnull
    private final MultiMap<PsiElement, consulo.localize.LocalizeValue> conflicts;
    @Nonnull
    ...
    public void detectInsideReferencesVisibilityProblems(@Nonnull List<RsMoveReferenceInfo> insideReferences) {
    ...
    private void updateItemsToMakePublic(@Nonnull List<RsMoveReferenceInfo> insideReferences) {
    ...
    public void detectOutsideReferencesVisibilityProblems(@Nonnull List<RsMoveReferenceInfo> outsideReferences) {
        detectReferencesVisibilityProblems(outsideReferences);
        detectPrivateFieldOrMethodOutsideReferences();
    }

    private void detectReferencesVisibilityProblems(@Nonnull List<RsMoveReferenceInfo> references) {
    ...
    private void detectPrivateFieldOrMethodInsideReferences() {
        ...
        consulo.language.psi.PsiFile sourceFile = sourceMod.getContainingFile();
    ...
    private void checkVisibilityForInsideReference(@Nonnull RsElement referenceElement, @Nonnull RsVisible target) {
    ...
    private void detectPrivateFieldOrMethodReferences(
        @Nonnull List<RsElement> elementsToCheck,
        @Nonnull java.util.function.BiConsumer<RsElement, RsVisible> checkVisibility
    ) {
    ...
    private void checkTraitImplIsCoherentAfterMove(@Nonnull RsImplItem impl) {
        if (!RsMoveUtil.checkOrphanRules(impl, element ->
            ...
            conflicts.putValue(impl, consulo.localize.LocalizeValue.of("Orphan rules check failed for trait implementation after move"));
        }
    }

    private void checkStructIsMovedTogetherWithInherentImpl(
        @Nonnull Set<RsStructOrEnumItemElement> structsToMove,
        @Nonnull Set<RsImplItem> inherentImplsToMove
    ) {
        for (RsImplItem impl : inherentImplsToMove) {
            ...
            if (struct != null && !structsToMove.contains(struct)) {
                ...
                conflicts.putValue(impl, consulo.localize.LocalizeValue.of("Inherent implementation should be moved together with " + structDescription));
            }
        }
    }
    ...
    private void checkInherentImplIsMovedTogetherWithStruct(
        ...
        Map<consulo.language.psi.PsiFile, List<RsStructOrEnumItemElement>> groupedByFile = structsToMove.stream()
            .collect(Collectors.groupingBy(PsiElement::getContainingFile));

        for (Map.Entry<consulo.language.psi.PsiFile, List<RsStructOrEnumItemElement>> entry : groupedByFile.entrySet()) {
    ...
    public static void addVisibilityConflict(
        @Nonnull MultiMap<PsiElement, consulo.localize.LocalizeValue> conflicts,
        @Nonnull RsElement reference,
        @Nonnull RsElement target
    ) {
        ...
        conflicts.putValue(reference, consulo.localize.LocalizeValue.of(StringUtil.capitalize(message)));
    }
}
