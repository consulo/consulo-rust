...
public class RsExtractEnumVariantProcessor extends BaseRefactoringProcessor {
    ...
    public RsExtractEnumVariantProcessor(@Nonnull Project project, @Nonnull Editor editor, @Nonnull RsEnumVariant ctx) {
        super(project);
        myEditor = editor;
        myCtx = ctx;
    }
    ...
    private PsiElement replaceUsage(
        ...
        boolean isTuple,
        @Nonnull PsiElement toBeReplaced
    ) {
        if (isTuple) {
    ...
    protected consulo.localize.LocalizeValue getCommandName() {
        ...
        return consulo.localize.LocalizeValue.of(RsBundle.message("command.name.extracting.variant", name));
    }
