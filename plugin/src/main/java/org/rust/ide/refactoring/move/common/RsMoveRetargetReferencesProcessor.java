...
public class RsMoveRetargetReferencesProcessor {
    ...
    public void retargetReferences(@Nonnull List<RsMoveReferenceInfo> referencesAll) {
    ...
    private void retargetReferenceDirectly(@Nonnull RsMoveReferenceInfo reference) {
        RsPath pathNew = reference.getPathNew();
        if (pathNew == null) return;
        replacePathOld(reference, pathNew);
    }

    private boolean tryRetargetReferenceKeepExistingStyle(@Nonnull RsMoveReferenceInfo reference) {
        RsPath pathOld = reference.getPathOld();
        RsPath pathNew = reference.getPathNew();
    ...
    private boolean doRetargetReferenceKeepExistingStyle(
        @Nonnull RsMoveReferenceInfo reference,
        @Nonnull String[] pathNewSegments,
        ...
        consulo.language.psi.PsiElement baseResolved = basePath.getReference() != null ? basePath.getReference().resolve() : null;
        RsPath usePathAsPath = RsMoveUtil.toRsPath(usePath, codeFragmentFactory, containingMod);
        consulo.language.psi.PsiElement elementToImport = usePathAsPath != null && usePathAsPath.getReference() != null
    ...
    private int adjustPathNewNumberSegments(@Nonnull RsMoveReferenceInfo reference, int numberSegments) {
    ...
    private void replacePathOld(@Nonnull RsMoveReferenceInfo reference, @Nonnull RsPath pathNew) {
        RsPath pathOld = reference.getPathOld();
        RsElement pathOldOriginal = reference.getPathOldOriginal();
    ...
    private void replacePathOldInPatIdent(@Nonnull RsPatIdent pathOldOriginal, @Nonnull RsPath pathNew) {
        ...
        consulo.language.psi.PsiElement patBindingNew = psiFactory.createIdentifier(referenceName);
        pathOldOriginal.getPatBinding().getIdentifier().replace(patBindingNew);
    }

    private boolean replacePathOldWithTypeArguments(@Nonnull RsPath pathOldOriginal, @Nonnull String pathNewText) {
        ...
        if (pathNew == null) return false;
        List<consulo.language.psi.PsiElement> elements = new ArrayList<>();
    ...
    private boolean tryReplacePathOldInUseGroup(@Nonnull RsPath pathOld, @Nonnull RsPath pathNew) {
    ...
    private void insertUseItemAndCopyAttributes(@Nonnull String useSpeckText, @Nonnull RsUseItem existingUseItem) {
    ...
    private void addImport(@Nonnull RsElement context, @Nonnull String usePath, @Nullable String alias) {
