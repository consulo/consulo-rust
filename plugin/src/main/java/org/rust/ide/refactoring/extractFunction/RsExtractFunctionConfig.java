...
public class RsExtractFunctionConfig extends RsFunctionSignatureConfig {
    ...
    private RsExtractFunctionConfig(
        ...
        @Nonnull OutputVariables outputVariables,
        @Nullable ControlFlow controlFlow,
        @Nonnull ReturnKind returnKind,
        @Nonnull String name,
    ...
    @Nullable
    public static RsExtractFunctionConfig create(@Nonnull PsiFile file, int start, int end) {
        ...
        if (lastElement != null) {
            consulo.language.ast.IElementType elementType = lastElement.getNode().getElementType();
    ...
    @Nullable
    private static RsExtractFunctionConfig doCreate(@Nonnull PsiFile file, int start, int end) {
        ...
        for (RsPatBinding binding : letBindings) {
            ...
            List<consulo.language.psi.PsiReference> allRefs = new ArrayList<>(
                ReferencesSearch.search(binding, new LocalSearchScope(fn)).findAll()
            );
            List<consulo.language.psi.PsiReference> targets = allRefs.stream()
