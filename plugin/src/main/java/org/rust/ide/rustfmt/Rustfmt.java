...
import org.rust.cargo.api.workspace.CargoWorkspace.Edition;
...
public class Rustfmt extends RustupComponent {
    ...
    public GeneralCommandLine createCommandLine(CargoProject cargoProject, Document document) {
        VirtualFile file = consulo.document.FileDocumentManager.getInstance().getFile(document);
        if (file == null) return null;
        if (RsFile.isNotRustFile(file) || !file.isValid()) return null;

        Project project = cargoProject.getProject();
        org.rust.cargo.api.settings.RustfmtProjectSettingsService settings = RsProjectSettingsServiceUtil.getRustfmtSettings(project);
        ...
        addArgument(cleanArguments, "edition", () -> {
            ...
            Edition edition = ReadAction.compute(() -> {
                consulo.language.psi.PsiFile psiFile = org.rust.openapiext.OpenApiUtil.toPsiFile(file, project);
                if (psiFile != null) {
        ...
        return createBaseCommandLine(cleanArguments, org.rust.cargo.project.model.CargoProjectLocator.getWorkingDirectory(cargoProject), settings.getEnvs());
    }
    ...
    public RsResult<Void, RsProcessExecutionException> reformatCargoProject(
        ...
        return org.rust.openapiext.OpenApiUtil.computeWithCancelableProgress(
            ...
            () -> {
                RsToolchainBase tc = RsToolchainLocator.getToolchain(project);
                if (tc == null) return new RsResult.Ok<Void, RsProcessExecutionException>(null);
                Cargo cargo = Cargo.cargoOrWrapper(tc, org.rust.cargo.project.model.CargoProjectLocator.getWorkingDirectory(cargoProject));
