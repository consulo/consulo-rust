...
public final class IntentionInMacroUtil {
    ...
    public static void runActionInsideMacroExpansionCopy(
        ...
        @Nonnull java.util.function.BiFunction<Editor, PsiElement, Boolean> action
    ) {
        PsiFile expansionFile = expandedElement.getContainingFile();
    ...
    private static void finishActionInMacroExpansionCopy(
        ...
        if (editorCopy != null) {
            ...
            consulo.codeEditor.Caret editorCopyCaret = editorCopy.getCaretModel().getCurrentCaret();
    ...
    private static void markRangesToReformat(List<RangeMarker> mappedChanges, PsiFile originalFile) {
        ...
        if (someLeaf != null) {
            ...
            if (someLeaf instanceof PsiWhiteSpaceImpl) {
                ...
                boolean isMarkedToReformatBefore = someLeaf.getNode() instanceof consulo.language.impl.ast.TreeElement
                    && CodeEditUtil.isMarkedToReformatBefore((consulo.language.impl.ast.TreeElement) someLeaf.getNode());
                someLeaf.replace(ws);
                if (prevLeaf != null) {
