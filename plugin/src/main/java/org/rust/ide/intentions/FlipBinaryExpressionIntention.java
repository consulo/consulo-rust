...
public class FlipBinaryExpressionIntention extends RsElementBaseIntentionAction<RsBinaryExpr> {
    ...
    public consulo.localize.LocalizeValue getText() {
        return consulo.localize.LocalizeValue.of(RsBundle.message("intention.name.flip.binary.expression"));
        }
    ...
    public RsBinaryExpr findApplicableContext(Project project, Editor editor, PsiElement element) {
        ...
        if (COMMUNICATIVE_OPERATORS.contains(opType)) {
            setText(consulo.localize.LocalizeValue.of(RsBundle.message("intention.name.flip", opText)));
        } else if (CHANGE_SEMANTICS_OPERATORS.contains(opType)) {
            setText(consulo.localize.LocalizeValue.of(RsBundle.message("intention.name.flip.changes.semantics", opText)));
        } else if (COMPARISON_OPERATORS.contains(opType)) {
            setText(consulo.localize.LocalizeValue.of(RsBundle.message("intention.name.flip.to", opText, flippedOp(opText))));
        } else {
            return null;
        }
        return binaryExpr;
