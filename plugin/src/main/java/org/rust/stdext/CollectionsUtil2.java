...
public final class CollectionsUtil2 {
     ...
     */
    @Nonnull
    public static <T> Iterable<T> chain(@Nonnull List<T> first, @Nonnull List<T> other) {
     ...
     */
    @Nonnull
    public static <T, R> List<R> mapToMutableList(@Nonnull Iterable<T> iterable, @Nonnull Function<T, R> transform) {
     ...
     */
    @Nonnull
    public static <T, R> Set<R> mapToSet(@Nonnull Iterable<T> iterable, @Nonnull Function<T, R> transform) {
        int size = CollectionsUtil.collectionSizeOrDefault(iterable, 10);
     ...
     */
    @Nonnull
    public static <T, R> Set<R> mapNotNullToSet(@Nonnull Iterable<T> iterable, @Nonnull Function<T, R> transform) {
        int size = CollectionsUtil.collectionSizeOrDefault(iterable, 10);
        Set<R> result = new HashSet<>(CollectionsUtil.mapCapacity(size));
        for (T item : iterable) {
     ...
     */
    @Nonnull
    public static <E extends Enum<E>> EnumSet<E> enumSetOf(@Nonnull Class<E> enumClass) {
     ...
     */
    @Nonnull
    public static <T> Iterable<LookbackValue<T>> withPrevious(@Nonnull Iterable<T> iterable) {
     ...
     */
    @Nonnull
    public static <T> Iterable<WithNextValue<T>> withNext(@Nonnull Iterable<T> iterable) {
    ...
    public static final class LookbackValue<T> {
        ...
        public LookbackValue(@Nonnull T current, @Nullable T previous) {
            myCurrent = current;
            myPrevious = previous;
        }
    ...
    public static final class WithNextValue<T> {
        ...
        public WithNextValue(@Nonnull T current, @Nullable T next) {
            myCurrent = current;
            myNext = next;
        }
    ...
    private static final class LookbackIterator<T> implements Iterator<LookbackValue<T>> {
        ...
        LookbackIterator(@Nonnull Iterator<T> iterator) {
            myIterator = iterator;
            myPrevious = null;
        }
    ...
    private static final class WithNextIterator<T> implements Iterator<WithNextValue<T>> {
        ...
        WithNextIterator(@Nonnull Iterator<T> iterator) {
            myIterator = iterator;
            myInitialized = false;
        }
