...
public class LocationMap<V> implements Map<MirLocation, V> {
    ...
    public LocationMap(@Nonnull MirBody body) {
    ...
    private int getStatementsSize(@Nonnull Object block) {
    ...
    @Nonnull
    public V getOrPut(@Nonnull MirLocation key, @Nonnull Supplier<V> defaultValue) {
        V value = get(key);
        if (value == null) {
    ...
    @Override
    public void putAll(@Nonnull Map<? extends MirLocation, ? extends V> from) {
        throw new UnsupportedOperationException();
    }
    ...
    @Override
    @Nullable
    public V put(@Nonnull MirLocation key, @Nullable V value) {
