...
public class CrateGraphServiceImpl implements CrateGraphService {
    ...
    public CrateGraphServiceImpl(@Nonnull Project project) {
        ...
        this.crateGraphCachedValue = consulo.application.util.CachedValuesManager.getManager(project).createCachedValue(() -> {
            CrateGraph result = buildCrateGraph(CargoProjectsService.getInstance(project).getAllProjects());
    ...
    @Nullable
    @Override
    public Crate findCrateByRootMod(@Nonnull VirtualFile rootModFile) {
        org.rust.openapiext.OpenApiUtil.checkReadAccessAllowed();
    ...
    private static class CrateGraph {
        ...
        CrateGraph(@Nonnull List<Crate> topSortedCrates, @Nonnull Int2ObjectMap<Crate> idToCrate) {
    ...
    @Nonnull
    private static CrateGraph buildCrateGraph(@Nonnull Collection<CargoProject> cargoProjects) {
    ...
    private static class CrateGraphBuilder {
        ...
        @Nullable
        CargoBasedCrate lowerPackage(@Nonnull ProjectPackage pkg) {
        ...
        private void replaceProjectAndTarget(@Nonnull ReplaceProjectAndTarget ctx) {
        ...
        @Nonnull
        private LoweredPackageDependencies lowerPackageDependencies(@Nonnull ProjectPackage pkg) {
        ...
        @Nonnull
        private static SplitDependencies classify(@Nonnull Collection<CargoWorkspace.Dependency> dependencies) {
        ...
        private List<Crate.Dependency> lowerDependencies(
            @Nonnull Iterable<CargoWorkspace.Dependency> deps,
            @Nonnull ProjectPackage pkg
        ) {
            try {
        ...
        private LoweredAndCyclicDependencies lowerDependenciesWithCycles(
            @Nonnull Collection<CargoWorkspace.Dependency> devDependencies,
            @Nonnull ProjectPackage pkg
        ) {
        ...
        private void lowerNonLibraryCratesLater(@Nonnull NonLibraryCrates ctx) {
        ...
        private void lowerNonLibraryCrates(@Nonnull NonLibraryCrates ctx) {
    ...
    private static void checkInvariants(@Nonnull Crate crate) {
    ...
    private static void assertTopSorted(@Nonnull Iterable<? extends Crate> crates) {
        if (!org.rust.openapiext.OpenApiUtil.isUnitTestMode()) return;
    ...
    private static Map<String, FeatureState> mergeFeatures(
        @Nonnull Map<String, FeatureState> features1,
        @Nonnull Map<String, FeatureState> features2
    ) {
    ...
    private static class ProjectPackage {
        @Nonnull final CargoProject project;
        @Nonnull final CargoWorkspace.Package pkg;
        @Nonnull final Path rootDirectory;

        ProjectPackage(@Nonnull CargoProject project, @Nonnull CargoWorkspace.Package pkg, @Nonnull Path rootDirectory) {
            this.project = project;
            this.pkg = pkg;
    ...
    private static class CyclicGraphException extends RuntimeException {
        ...
        CyclicGraphException(@Nonnull String crateName) {
            super("Cyclic graph detected");
            this.stack = new ArrayList<>();
            this.stack.add(crateName);
        }

        void pushCrate(@Nonnull String crateName) {
            stack.add(crateName);
        }
