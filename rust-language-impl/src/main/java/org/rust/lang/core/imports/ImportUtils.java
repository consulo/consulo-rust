...
public final class ImportUtils {
     ...
     */
    public static boolean isStd(@Nonnull Crate crate) {
     ...
     */
    public static boolean isCore(@Nonnull Crate crate) {
        return crate.getOrigin() == PackageOrigin.STDLIB
     ...
     */
    @Nullable
    public static <T extends RsElement> T lastElement(@Nonnull List<T> list) {
     ...
     */
    public static void importCandidate(@Nonnull ImportCandidate candidate, @Nonnull RsElement context) {
     ...
     */
    @Nonnull
    public static RsMod createVirtualImportContext(@Nonnull RsElement element) {
     ...
     */
    public static void insertUseItem(@Nonnull RsMod mod, @Nonnull RsPsiFactory psiFactory, @Nonnull RsUseItem useItem) {
        if (tryInsertUseItemAtCorrectLocation(mod, useItem)) return;

        // Handle case when mod is empty or has no uses / extern crates
        consulo.language.psi.PsiElement firstItem = RsModUtil.getFirstItem(mod);
    ...
    private static boolean tryInsertUseItemAtCorrectLocation(@Nonnull RsMod mod, @Nonnull RsUseItem useItem) {
        ...
        if (uses.isEmpty()) {
            ...
            consulo.language.psi.PsiElement anchor = externCrates.get(externCrates.size() - 1);
        ...
        if (anchorBefore != null) {
            consulo.language.psi.PsiElement addedItem = mod.addAfter(useItem, anchorBefore.getUseItem());
            mod.addBefore(newline.createNewline(), addedItem);
        } else if (anchorAfter != null) {
            consulo.language.psi.PsiElement addedItem = mod.addBefore(useItem, anchorAfter.getUseItem());
