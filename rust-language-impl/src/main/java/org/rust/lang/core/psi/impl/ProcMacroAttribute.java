...
import org.rust.lang.core.psi.impl.CustomAttributes;
import org.rust.lang.core.psi.impl.KnownProcMacroKind;
...
public abstract class ProcMacroAttribute<T extends RsMetaItemPsiOrStub> {
    ...
    public static class Derive<T extends RsMetaItemPsiOrStub> extends ProcMacroAttribute<T> {
        ...
        public Derive(@Nonnull Iterable<T> derives) {
            myDerives = derives;
        }
    ...
    public static class Attr<T extends RsMetaItemPsiOrStub> extends ProcMacroAttribute<T> {
        ...
        public Attr(@Nonnull T attr, int index) {
            myAttr = attr;
            myIndex = index;
        }
    ...
    public static <T extends RsMetaItemPsiOrStub> List<ProcMacroAttribute<T>> getProcMacroAttributeWithoutResolve(
        @Nonnull RsAttrProcMacroOwnerPsiOrStub<T> owner,
        ...
        if (!ignoreProcMacrosDisabled && !ProcMacroFlags.isAnyEnabled()) return Collections.emptyList();
        if (stub != null) {
            if (!stub.getMayHaveCustomAttrs()) {
                if (stub.getMayHaveCustomDerive() && RsProcMacroPsiUtil.canOwnDeriveAttrs(owner)) {
                    if (!ignoreProcMacrosDisabled && !ProcMacroFlags.isDeriveEnabled()) return Collections.emptyList();
                    if (withDerives) {
        ...
        for (T meta : queryAttributes.getMetaItems()) {
            if ("derive".equals(meta.getName())) {
                if (RsProcMacroPsiUtil.canOwnDeriveAttrs(owner)) {
                    if (!ignoreProcMacrosDisabled && !ProcMacroFlags.isDeriveEnabled()) break;
                    if (withDerives) {
            ...
            if (RsProcMacroPsiUtil.canBeProcMacroAttributeCallWithoutContextCheck(meta, customAttributes)
                && (ignoreProcMacrosDisabled || ProcMacroFlags.isAttrEnabled())) {
    ...
    public static ProcMacroAttribute<RsMetaItem> getProcMacroAttribute(
        ...
        return getProcMacroAttribute(owner, org.rust.lang.core.psi.ext.impl.RsDocAndAttributeOwnerUtil.getAttributeStub(owner), null, false, false);
    }
    ...
    @Nonnull
    public static List<KnownProcMacroKind> getHardcodedProcMacroAttributes(@Nonnull RsAttrProcMacroOwner owner) {
        List<KnownProcMacroKind> ignoredAttributes = new ArrayList<>();
        getProcMacroAttributeWithHardcoded(owner, org.rust.lang.core.psi.ext.impl.RsDocAndAttributeOwnerUtil.getAttributeStub(owner), null, false, false, ignoredAttributes);
