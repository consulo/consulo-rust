...
public final class RsProcMacroPsiUtil {
    ...
    public static boolean canBeInProcMacroCallBody(@Nonnull PsiElement psiElement) {
        if (!ProcMacroFlags.isAnyEnabled()) return false;
    ...
    private static boolean canHaveProcMacroCall(@Nonnull RsAttrProcMacroOwner item) {
        ...
        for (var meta : RsDocAndAttributeOwnerUtil.getTraversedRawAttributes(item, false).getMetaItems()) {
            if (canBeProcMacroAttributeCall(meta, CustomAttributes.EMPTY) && ProcMacroFlags.isAttrEnabled()) return true;
                ...
                if (args != null) {
                    for (var m : args.getMetaItemList()) {
                        if (canBeCustomDerive(m)) {
                            return ProcMacroFlags.isDeriveEnabled();
                        }
                    }
                }
            }
     ...
     */
    public static boolean canBeCustomDerive(@Nonnull RsMetaItem metaItem) {
    ...
    public static boolean canBeCustomDeriveWithoutContextCheck(@Nonnull RsMetaItemPsiOrStub metaItem) {
    ...
    public static boolean canBeProcMacroAttributeCall(
        ...
        @Nonnull CustomAttributes customAttrs
    ) {
    ...
    public static boolean canBeProcMacroAttributeCallWithoutContextCheck(
        @Nonnull RsMetaItemPsiOrStub metaItem,
        @Nonnull CustomAttributes customAttrs
    ) {
        String name = RsMetaItemUtil.getName(metaItem);
        if (name == null) {
     ...
     */
    public static boolean canBeProcMacroCall(@Nonnull RsMetaItem metaItem) {
     ...
     */
    public static boolean canFallBackAttrMacroToOriginalItem(@Nonnull RsAttrProcMacroOwnerStub item) {
    ...
    public static boolean canFallBackAttrMacroToOriginalItem(@Nonnull RsAttrProcMacroOwner item) {
    ...
    public static boolean canOwnDeriveAttrs(@Nonnull RsAttrProcMacroOwnerPsiOrStub<?> item) {
