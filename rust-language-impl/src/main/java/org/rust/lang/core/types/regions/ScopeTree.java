...
public class ScopeTree {
    ...
    public void recordScopeParent(@Nonnull Scope childScope, @Nullable ScopeInfo parentInfo) {
        if (parentInfo != null) {
    ...
    @Nullable
    public Scope getDestructionScope(@Nonnull RsElement element) {
        return myDestructionScopes.get(element);
    }

    public void recordVarScope(@Nonnull RsPatBinding variable, @Nonnull Scope lifetime) {
    ...
    public void recordVarScope(@Nonnull RsSelfParameter variable, @Nonnull Scope lifetime) {
        if (variable == lifetime.getElement()) {
            throw new IllegalStateException("Variable cannot be its own scope element");
        }
        myVarMap.put(variable, lifetime);
    ...
    public void recordRvalueCandidate(@Nonnull RsExpr expr, @Nonnull RvalueCandidateType candidateType) {
    ...
    @Nullable
    public Scope getEnclosingScope(@Nonnull Scope scope) {
    ...
    @Nullable
    public Scope getVariableScope(@Nonnull RsPatBinding variable) {
        return myVarMap.get(variable);
    }

    /** Returns the lifetime of the local variable, if any. */
    @Nullable
    public Scope getVariableScope(@Nonnull RsSelfParameter variable) {
        return myVarMap.get(variable);
    }
     ...
     */
    @Nonnull
    public Scope getLowestCommonAncestor(@Nonnull Scope scope1, @Nonnull Scope scope2) {
