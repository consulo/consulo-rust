...
import org.rust.lang.core.psi.impl.RsStability;
...
public class QueryAttributes<T extends RsMetaItemPsiOrStub> {
    ...
    public QueryAttributes(@Nonnull Iterable<T> metaItems) {
        this.metaItems = metaItems;
    }

    @SuppressWarnings("unchecked")
    public QueryAttributes(@Nonnull Stream<? extends T> metaItems) {
    ...
    public boolean hasAnyOfAttributes(@Nonnull String... names) {
        for (T item : metaItems) {
    ...
    // `#[attributeName]`
    public boolean hasAtomAttribute(@Nonnull String attributeName) {
        for (T item : metaItems) {
    ...
    // `#[attributeName(arg)]`
    public boolean hasAttributeWithArg(@Nonnull String attributeName, @Nonnull String arg) {
        for (T item : metaItems) {
            if (attributeName.equals(RsMetaItemUtil.getName(item))) {
    ...
    // `#[attributeName = "value"]`
    public boolean hasAttributeWithValue(@Nonnull String attributeName, @Nonnull String value) {
        for (T item : metaItems) {
    ...
    // `#[attributeName(arg)]`
    @Nullable
    public String getFirstArgOfSingularAttribute(@Nonnull String attributeName) {
        T singular = null;
        for (T item : metaItems) {
            if (attributeName.equals(RsMetaItemUtil.getName(item))) {
    ...
    public boolean hasAttributeWithKeyValue(@Nonnull String attributeName, @Nonnull String key, @Nonnull String value) {
        for (T item : metaItems) {
            if (attributeName.equals(RsMetaItemUtil.getName(item))) {
                for (RsMetaItemPsiOrStub sub : item.getMetaItemArgsList()) {
    ...
    @Nullable
    public String lookupStringValueForKey(@Nonnull String key) {
        String result = null;
        for (T item : metaItems) {
    ...
    @Nonnull
    public Iterable<T> attrsByName(@Nonnull String name) {
        return () -> {
            java.util.Iterator<T> iter = metaItems.iterator();
            ...
            if ("derive".equals(meta.getName())) {
                ...
                if (args != null) {
                    for (T derive : (Iterable<T>) (Iterable<?>) args.getMetaItemList()) {
                        if (org.rust.lang.core.psi.impl.RsProcMacroPsiUtil.canBeCustomDeriveWithoutContextCheck(derive)) {
                            result.add(derive);
                        }
                    }
                }
    ...
    // #[stable], #[unstable]
    @Nullable
    public org.rust.lang.core.psi.impl.RsStability getStability() {
        for (T item : metaItems) {
            String name = RsMetaItemUtil.getName(item);
            if ("stable".equals(name)) return org.rust.lang.core.psi.impl.RsStability.Stable;
            if ("unstable".equals(name)) return org.rust.lang.core.psi.impl.RsStability.Unstable;
        }
        return null;
    }
