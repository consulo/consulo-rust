...
import org.rust.lang.core.psi.ext.impl.RsIndexExprExtUtil;
...
public class RvalueScopes {
    ...
    @Nullable
    public Scope temporaryScope(@Nonnull ScopeTree regionScopeTree, @Nonnull RsExpr expr) {
    ...
    public void recordRvalueScope(@Nonnull RsElement element, @Nullable Scope lifetime) {
        if (lifetime != null) {
    ...
    @Nonnull
    public static RvalueScopes resolveRvalueScopes(@Nonnull ScopeTree scopeTree) {
    ...
    private static void recordRvalueScopeRec(@Nonnull RvalueScopes rvalueScopes, @Nonnull RsExpr expr, @Nullable Scope lifetime) {
        RsExpr exprVar = expr;
        while (true) {
