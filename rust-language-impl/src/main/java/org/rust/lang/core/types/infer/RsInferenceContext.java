...
import org.rust.lang.utils.RsTypeMismatch;
...
public class RsInferenceContext implements RsInferenceData {
    ...
    public final List<RsInferenceDiagnostic> diagnostics = new ArrayList<>();
    ...
    public RsInferenceContext(@Nonnull Project project,
    ...
    @Nullable
    public <T> T probe(@Nonnull Supplier<T> action) {
    ...
    @Nullable
    public <T> T commitIfNotNull(@Nonnull Supplier<T> action) {
        Snapshot snapshot = startSnapshot();
        T result = action.get();
        if (result == null) {
            snapshot.rollback();
    ...
    @Nonnull
    public RsInferenceResult infer(@Nonnull RsInferenceContextOwner element) {
        ...
        diagnostics.replaceAll(d -> {
            if (d instanceof RsTypeMismatch) {
                return fullyResolve((RsTypeMismatch) d);
            }
            return d;
        });
    ...
    private void inferPath(@Nonnull RsPath element) {
    ...
    private void inferOther(@Nonnull RsInferenceContextOwner element) {
        Ty retTy = null;
        RsExpr expr = null;
    ...
    private void performPathsRefinement(@Nonnull ImplLookup lookup) {
    ...
    public boolean isTypeInferred(@Nonnull RsExpr expr) {
        return myExprTypes.containsKey(expr);
    }

    // --- Write methods ---

    public void writeExprTy(@Nonnull RsExpr psi, @Nonnull Ty ty) {
    ...
    public void writeExpectedExprTy(@Nonnull RsExpr psi, @Nonnull Ty ty) {
        myExpectedExprTypes.put(psi, new ExpectedType(ty));
    }

    public void writeExpectedExprTyCoercable(@Nonnull RsExpr psi) {
        myExpectedExprTypes.computeIfPresent(psi, (k, v) -> v.withCoercable(true));
    }

    public void writePath(@Nonnull RsPathExpr path, @Nonnull List<ResolvedPath> resolved) {
        myResolvedPaths.put(path, resolved);
    }

    public void writePathSubst(@Nonnull RsPathExpr path, @Nonnull Substitution subst) {
    ...
    public void writeResolvedMethod(@Nonnull RsMethodCall call, @Nonnull List<MethodResolveVariant> resolvedTo) {
        myResolvedMethods.put(call, new InferredMethodCallInfo(resolvedTo));
    }

    public void writeResolvedMethodSubst(@Nonnull RsMethodCall call, @Nonnull Substitution subst, @Nonnull TyFunctionBase ty) {
        InferredMethodCallInfo info = myResolvedMethods.get(call);
        if (info != null) {
            info.setSubst(subst);
            info.setType(ty);
    ...
    public void writeResolvedField(@Nonnull RsFieldLookup lookup, @Nonnull List<RsElement> resolvedTo) {
        myResolvedFields.put(lookup, resolvedTo);
    }

    public void registerPathRefinement(@Nonnull RsPathExpr path, @Nonnull TraitRef traitRef) {
        myPathRefinements.add(new AbstractMap.SimpleEntry<>(path, traitRef));
    }

    public void registerMethodRefinement(@Nonnull RsMethodCall path, @Nonnull TraitRef traitRef) {
        myMethodRefinements.add(new AbstractMap.SimpleEntry<>(path, traitRef));
    }

    public void addDiagnostic(@Nonnull PsiElement element, @Nonnull RsInferenceDiagnostic diagnostic) {
        if (element.getContainingFile().isPhysical()) {
            diagnostics.add(diagnostic);
        }
    }

    public void applyAdjustment(@Nonnull RsElement expr, @Nonnull Adjustment adjustment) {
        applyAdjustments(expr, Collections.singletonList(adjustment));
    }

    public void applyAdjustments(@Nonnull RsElement expr, @Nonnull List<Adjustment> adjustment) {
    ...
    public void writeOverloadedOperator(@Nonnull RsExpr expr) {
        myOverloadedOperators.add(expr);
    }

    public void reportTypeMismatch(@Nonnull RsElement element, @Nonnull Ty expected, @Nonnull Ty actual) {
        addDiagnostic(element, new RsTypeMismatch(element, expected, actual));
    }

    // --- Type combination ---

    public boolean canCombineTypes(@Nonnull Ty ty1, @Nonnull Ty ty2) {
        return probe(() -> combineTypes(ty1, ty2).isOk());
    }

    @Nonnull
    public RsResult<Object, TypeError> combineTypes(@Nonnull Ty ty1, @Nonnull Ty ty2) {
        return combineTypesResolved(shallowResolve(ty1), shallowResolve(ty2));
    }

    @Nonnull
    private RsResult<Object, TypeError> combineTypesResolved(@Nonnull Ty ty1, @Nonnull Ty ty2) {
    ...
    @Nonnull
    private RsResult<Object, TypeError> combineTyVar(@Nonnull TyInfer.TyVar ty1, @Nonnull Ty ty2) {
    ...
    @Nonnull
    private RsResult<Object, TypeError> combineIntOrFloatVar(@Nonnull TyInfer ty1, @Nonnull Ty ty2) {
    ...
    @Nonnull
    public RsResult<Object, TypeError> combineTypesNoVars(@Nonnull Ty ty1, @Nonnull Ty ty2) {
    ...
    @Nonnull
    private RsResult<Object, TypeError> combineFunctions(@Nonnull TyFunctionBase ty1, @Nonnull TyFunctionBase ty2) {
    ...
    @Nonnull
    public RsResult<Object, TypeError> combineConsts(@Nonnull Const const1, @Nonnull Const const2) {
    ...
    @Nonnull
    private RsResult<Object, TypeError> combineConstVar(@Nonnull CtInferVar const1, @Nonnull Const const2) {
    ...
    @Nonnull
    private RsResult<Object, TypeError> combineConstsNoVars(@Nonnull Const c1, @Nonnull Const c2) {
    ...
    public <T extends RsElement> boolean combineBoundElements(@Nonnull BoundElement<T> be1, @Nonnull BoundElement<T> be2) {
    ...
    @Nonnull
    public RsResult<CoerceOk, TypeError> tryCoerce(@Nonnull Ty inferred, @Nonnull Ty expected) {
    ...
    private RsResult<CoerceOk, TypeError> coerceFromSafeFn(
        @Nonnull TyFunctionBase inferred,
        @Nonnull TyFunctionPointer expected,
    ...
    @Nullable
    private CoerceOk coerceUnsized(@Nonnull Ty source, @Nonnull Ty target) {
    ...
    @Nonnull
    private RsResult<CoerceOk, TypeError> coerceReference(@Nonnull TyReference inferred, @Nonnull TyReference expected) {
    ...
    private boolean coerceMutability(@Nonnull Mutability from, @Nonnull Mutability to) {
    ...
    @Nonnull
    public <T extends TypeFoldable<T>> T shallowResolve(@Nonnull T value) {
        return value.foldWith(myShallowResolver);
    }

    @Nonnull
    public Ty resolveTypeVarsWithObligations(@Nonnull Ty ty) {
    ...
    @Nonnull
    public <T extends TypeFoldable<T>> T resolveTypeVarsIfPossible(@Nonnull T value) {
        return value.foldWith(myOpportunisticVarResolver);
    }

    @Nonnull
    public <T extends TypeFoldable<T>> T fullyResolve(@Nonnull T value) {
        return value.foldWith(myFullTypeResolver);
    }

    @Nonnull
    public <T extends TypeFoldable<T>> T fullyResolveWithOrigins(@Nonnull T value) {
        return value.foldWith(myFullTypeWithOriginsResolver);
    }

    @Nonnull
    public Ty typeVarForParam(@Nonnull TyTypeParameter ty) {
        return new TyInfer.TyVar(ty);
    }

    @Nonnull
    public Const constVarForParam(@Nonnull CtConstParameter c) {
        return new CtInferVar(c);
    }

    @Nonnull
    public <T extends TypeFoldable<T>> T fullyNormalizeAssociatedTypesIn(@Nonnull T ty) {
    ...
    @Nonnull
    public <T extends TypeFoldable<T>> TyWithObligations<T> normalizeAssociatedTypesIn(@Nonnull T ty) {
        return normalizeAssociatedTypesIn(ty, 0);
    }

    @Nonnull
    public <T extends TypeFoldable<T>> TyWithObligations<T> normalizeAssociatedTypesIn(@Nonnull T ty, int recursionDepth) {
        List<Obligation> obligations = new ArrayList<>();
        T normTy = ty.foldWith(new TypeFolder() {
            @Nonnull
            @Override
            public Ty foldTy(@Nonnull Ty t) {
    ...
    @Nonnull
    private TyWithObligations<Ty> normalizeProjectionType(@Nonnull TyProjection projectionTy, int recursionDepth) {
    ...
    @Nullable
    public TyWithObligations<Ty> optNormalizeProjectionType(@Nonnull TyProjection projectionTy, int recursionDepth) {
    ...
    @Nullable
    private TyWithObligations<Ty> optNormalizeProjectionTypeResolved(@Nonnull TyProjection projectionTy, int recursionDepth) {
    ...
    @Nonnull
    private TyWithObligations<Ty> pruneCacheValueObligations(@Nonnull TyWithObligations<Ty> ty) {
    ...
    private <T extends TypeFoldable<T>> boolean hasUnresolvedTypeVars(@Nonnull T ty) {
    ...
    public <T extends TypeFoldable<T>> boolean hasResolvableTypeVars(@Nonnull T ty) {
        return FoldUtil.visitInferTys(ty, it -> !it.equals(shallowResolve(it)));
    }

    public boolean isTypeVarAffected(@Nonnull TyInfer.TyVar ty) {
        return myVarUnificationTable.findRoot(ty) != ty || myVarUnificationTable.findValue(ty) != null;
    }

    @Nonnull
    public Iterable<Obligation> instantiateBounds(@Nonnull List<Predicate> bounds, @Nonnull Substitution subst, int recursionDepth) {
    ...
    @Nonnull
    public Substitution instantiateBounds(@Nonnull RsGenericDeclaration element) {
        return instantiateBounds(element, null, SubstitutionUtil.EMPTY_SUBSTITUTION);
    }

    @Nonnull
    public Substitution instantiateBounds(@Nonnull RsGenericDeclaration element, @Nullable Ty selfTy, @Nonnull Substitution subst) {
    ...
    public boolean canEvaluateBounds(@Nonnull TraitImplSource source, @Nonnull Ty selfTy) {
    ...
    private boolean canEvaluateBoundsImpl(@Nonnull RsImplItem impl, @Nonnull Ty selfTy) {
    ...
    @Nonnull
    public Substitution instantiateMethodOwnerSubstitution(@Nonnull AssocItemScopeEntryBase<?> callee) {
        return instantiateMethodOwnerSubstitution(callee, null);
    }

    @Nonnull
    public Substitution instantiateMethodOwnerSubstitution(@Nonnull AssocItemScopeEntryBase<?> callee, @Nullable RsMethodCall methodCall) {
        return instantiateMethodOwnerSubstitutionImpl(callee.getSource(), callee.getSelfTy(), callee.getElement(), methodCall);
    }

    @Nonnull
    public Substitution instantiateMethodOwnerSubstitution(@Nonnull MethodPick callee) {
        return instantiateMethodOwnerSubstitution(callee, null);
    }

    @Nonnull
    public Substitution instantiateMethodOwnerSubstitution(@Nonnull MethodPick callee, @Nullable RsMethodCall methodCall) {
    ...
    public void convertPlaceDerefsToMutable(@Nonnull RsExpr receiver) {
    ...
    private class ShallowResolver implements TypeFolder {
        ...
        @Nonnull
        @Override
        public Const foldConst(@Nonnull Const c) {
        ...
        @Nonnull
        private Ty shallowResolveTy(@Nonnull Ty ty) {
