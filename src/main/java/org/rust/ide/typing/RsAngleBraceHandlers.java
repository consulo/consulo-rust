...
public final class RsAngleBraceHandlers {
    ...
    public static final RsBraceHandlers.BraceHandler ANGLE_BRACE_HANDLER = new RsBraceHandlers.BraceHandler() {
        ...
        @Override
        public boolean shouldComplete(@NotNull Editor editor) {
            int offset = editor.getCaretModel().getOffset();
            HighlighterIterator lexer = TypingUtil.createLexer(editor, offset - 1);
            if (lexer == null) return false;

            if (lexer.getTokenType() == RsElementTypes.COLONCOLON) return true;
            if (lexer.getTokenType() == RsElementTypes.IMPL) return true;
        ...
        @Override
        public int calculateBalance(@NotNull Editor editor) {
            ...
            while (!iterator.atEnd() && balance >= 0 && !INVALID_INSIDE_TOKENS.contains(iterator.getTokenType())) {
                if (iterator.getTokenType() == RsElementTypes.LT) balance++;
                else if (iterator.getTokenType() == RsElementTypes.GT) balance--;
                iterator.advance();
            }
            return balance;
        }

        private boolean isTypeLikeIdentifier(int offset, @NotNull Editor editor, @NotNull HighlighterIterator iterator) {
